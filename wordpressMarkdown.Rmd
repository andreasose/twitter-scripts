...
If you want to save these tweets for a later time, then use  `write.csv()` to write the tweets to your hard drive. Now that we have the tweets down, it is useful to remove any unneccesary fluff.  
...
After that, we remove all the so-called "stopwords"(words that do not carry any significant meaning to the topic), and convert the text into a Term Document Matrix. The TDM is then summed up so we get a data.frame of words arranged by how often they are used 
...
To clean the tweets, pass  `clinton_tweets <- tdmCreator(clinton_tweets)` and `trump_tweets <- tdmCreator(trump_tweets)` into R.  
