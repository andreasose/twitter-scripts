...
After that, we remove all the so-called "stopwords", convert the text into a Term Document Matrix, remove the least used words, before summing up how many time each word has been used. The top 10 words can then be plotted.
...
This function works in the same way, but takes two extra arguments specifiying if you want to stem the words and remove any stopwords. I have defaulted them to true in this example for both `clinton_tweets <- tdmCreator(clinton_tweets)` and `trump_tweets <- tdmCreator(trump_tweets)`.  
...
All done? Excellent, now we have a lot of data to work with. Let's re-run the previous analysis.
```{r}
democrat <- read.csv("democrats.csv")
republican <- read.csv("republicans.csv")
democrat <- textScrubber(democrat)
republican <- textScrubber(republican)

dem <- tdmCreator(democrat)
rep <- tdmCreator(republican)
...
democrat <- read.csv("democrats.csv")
republican <- read.csv("republicans.csv")
democrat$group <- as.factor("dem")
republican$group <- as.factor("rep")
both <- rbind(democrat, republican)
...
#ggplot(both, aes(day, colour = group)) + geom_bar()
...
We can see a difference in when they tweet  

![image](https://raw.githubusercontent.com/andreasose/twitter-scripts/master/densityWeek.png)  

This difference is especially marked on some of the weekdays.
...
```{r}
repDevice <- sort(device(republican), decreasing = T)
demDevice <- sort(device(democrat), decreasing = T)
...
That's some pretty big differences there between Trump and Hillary especially. Whereas Hillary seems to use Tweetdeck the most (probably to schedule tweets), Trump seems to live and die by his two phones.  
Republicans and Democrats are more similar, but note how Republicans seem to use scheduling services (TweetDeck and Hootsuite) more often. Also of note is the complete lack of Android phones used by the representatives. 