...
```
To run the function, type in `r clinton_tweets <- textScrubber(clinton_tweets)` and `r trump_tweets <- textScrubber(trump_tweets)`  
...
```
This function works in the same way, but takes two extra arguments specifiying if you want to stem the words and remove any stopwords. I have set them to respectively false and true in this example for both `r clinton_tweets <- tdmCreator(clinton_tweets)` and `r trump_tweets <- tdmCreator(trump_tweets)`.  
...
[image](C:/Users/user/Dropbox/R/Data/Twitter-scripts/index2.png)  
...
All done? Excellent, now we have a lot of data to work with. I suggest that you first run the text-scrubber.
